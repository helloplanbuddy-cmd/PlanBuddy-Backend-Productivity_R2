...
function requireInternalAccess(req, res, next) {
  ...
  // Security: default deny external traffic to /internal/*
...
// Route must be mounted BEFORE express.json() to preserve the raw Buffer.
app.post(
  '/webhooks/razorpay',
...
// 🔥 ENABLED: Protects against API abuse and DDoS
app.use('/api', globalLimiter);

// 🚀 PHASE 4: Backpressure middleware (request throttling)
// 🔥 ENABLED: Critical for preventing system collapse under load
...
app.use((req, res, next) => {
  ...
  monitoring.request_total.inc({ method: req.method, path: req.path, status: 'pending' });
  ...
  res.on('finish', () => {
    ...
    monitoring.request_total.inc({ method: req.method, path: req.path, status: res.statusCode });
  });
...
// Security-only Phase 1: lock /internal/*
app.use('/internal', requireInternalAccess, internalRoutes);
...
/** TEMP endpoints for load testing / verification **/

// ─── TEMP PING endpoint for load testing (no DB/Redis) ─────────────────────
app.get('/ping', (req, res) => res.send('ok'));

// ─── PID endpoint (security hardening) ─────────────────────────────────────
// /pid must not be accessible in non-development environments.
app.get('/pid', (req, res) => {
...
// ─── 404 handler ─────────────────────────────────────────────────────────────
// ✅ FIX API-001: Real health check with DB + Redis validation
...
console.log(`[boot] pid=${process.pid} port=${port} node_env=${env.NODE_ENV} is_prod=${env.IS_PROD}`);
...
server.on('error', (err) => {
  if (err && err.code === 'EADDRINUSE') {
    console.error(`[boot] EADDRINUSE: port ${port} already in use. Another process is already running.`);
    process.exit(1);
  }
...
process.on('SIGTERM', async () => {
  ...
  server.close(async () => {
    ...
    try {
      ...
      try {
        const db = require('./config/db');
        await db.pool.end(); // ✅ FIX DEP-002: db.end() does not exist, use db.pool.end()
